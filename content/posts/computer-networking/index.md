...
- [**回退N步(GBN, Go-Back-N)**](https://media.pearsoncmg.com/ph/esm/ecs_kurose_compnetwork_8/cw/content/interactiveanimations/go-back-n-protocol/index.html)
...
- [**选择重传(SR,Selective Repeat)**](https://media.pearsoncmg.com/ph/esm/ecs_kurose_compnetwork_8/cw/content/interactiveanimations/selective-repeat-protocol/index.html)
...
通过每个节点**广播**自身所连接链路的标识和开销，
使得所有节点都具有网络的同一的链路状态信息。

采用Dijkstra算法计算最小开销路径。

> 存在**链路震荡**的问题：发送链路通告时间随机化来避免

- 距离向量算法

迭代、分布式、异步的算法。

节点向邻居发送距离向量，在收到邻居向量时，采用Bellman-Ford方程来更新自己的向量。

> **Bellman-Ford方程：**
> $D_x(y) = \min_v\{c(x,v) + D_v(y)\}$
>
> $D_x(y)$表示从$x$到$y$的距离向量，
> $v$为节点$x$所有的邻居
> $c(x,v)$为$x$本地记录的开销，
> $D_v(y)$是接受到的邻居节点到$y$的开销。

> 在链路开销变化和链路故障时，存在**路由选择环路**的问题：增加毒性逆转
...
链路状态协议，用于自治系统(AS)内部，周期性&链路变化时广播链路状态

安全（MD5鉴别），多条开销相同的路径，单播&多播支持，单个AS中也可划分多层次

- BGP

连接互联网中的ISP，分为eBGP和iBGP。

**热土豆路由**：尽快将分组送出AS

可配置路由偏好

IP任播：可让每个用户从最靠近的服务器访问内容，广泛用于DNS请求

## SDN控制平面

- 基于流的转发：不同于传统的基于IP目的地址的转发，可根据任意首部字段进行配置匹配
- 数据平面与控制平面分离：数据平面由简单的交换机组成，根据流表进行转发
- 网络控制功能由逻辑上集中的SDN控制器和应用程序完成
- 可编程的网络：可修改SDN控制应用程序来应用不同的算法
...
上述功能主要都在网络适配器（网络接口卡，NIC）中以硬件方式实现。
...
奇校验：使得信息+校验bit中1的个数为奇数个；
偶校验：使得信息+校验bit中1的个数为偶数个；

- 二维奇偶校验

将初试 $d$ 比特信息划分成 $i$ 行 $j$ 列，每行每列都进行奇偶校验，共有 $i+j+1$ 个校验位。

可检测并纠正单比特差错；可检测任意双比特差错，但不能纠正

前向纠错能力（FEC）

- 校验和

将数据的字节作为16比特的整数进行求和，并将和的结果的反码作为校验和字段。

接收端只需要将所有字段按照16比特划分相加，若求和结果中存在0则表示数据出错。

- 循环冗余检测（CRC）

**模2运算**：加法不进位，减法不借位，即异或运算

**生成多项式 $G$**：`1011` $\leftrightarrow$ 
$1\cdot x^3 + 0\cdot x^2 + 0\cdot x^1 + 0\cdot x^0$

生成多项式 $G$ 的最高次数为 $r$，共 $r + 1$ 项；

校验码为 $r$ 位

校验码的值为 $R = \dfrac{D \cdot 2^r}{G}\text{取余数}$，不足 $r$ 位前面填$0$补足 $r$位。

能够检测小于 $r + 1$位的差错，以 $1-0.5^r$ 的概率检测出大于 $r + 1$ 位的突发错误
...
- CSMA
  - 在发送前进行载波侦听
  - 信道端到端传播时延越大，载波侦听节点越容易出现侦听失败导致碰撞

- CSMA/CD (colision detect)
  - 在发送前和发送时都进行载波侦听
  - 在检测到碰撞时立马停止传输
  - 采用二进制指数退避，一个退避单位时间为512位时
  - 最大效率 $\dfrac{1}{1+5 d_{prop}/d_{trans}}$

- CSMA/CA (colision avoid)
  - 开放空间电磁波衰减快，干扰大
  - 无法进行冲突检测(CD)：自身信号远大于其他节点信号，隐藏/暴露终端问题
  - 发送前事先进行冲突避免：监听信道，DIFS退避
  - 发送方：
    - 侦听：发送方先侦听信道，若空闲DIFS时间，则传输整个帧；
    - 退避：若在DIFS期间侦听到信道忙碌，则随机选择一个退避值，在空闲时-1，直到为0后进行发送。若未收到ACK，则增加退避值，重复退避；
  - 接收方：
    - 收到正确的帧后等待SIFS后回复ACK；
  - SIFS < DIFS 
  - RTS/CTS：避免隐藏终端，减少长数据帧下碰撞的开销
...
- 轮询协议（集中式）
  ...
  - 主节点单点故障
- 令牌传递协议（分布式）
  - 节点间以某种固定的顺序流转**令牌**特殊帧，持有令牌的人可发送
  - 单点故障

### 电缆接入网络

CMTS：电缆调制解调器端系统，与住宅调制解调器相连

上下行：FDM

上行信道：TDM，包含请求帧微时隙和数据帧微时隙

下行信道：发布请求结果和其他数据，无多路访问问题，只有CMTS一个发送方

## 交换局域网

### ARP

将IP地址转换为链路层地址，仅限于一个子网内部主机和路由接口的解析
